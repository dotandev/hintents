...
Examples:
  ...
  erst snapshot-diff --snapshot-a before.json --snapshot-b after.json --context 32`,
	...
	cmd.Flags().StringVar(&snapshotDiffAFlag, "snapshot-a", "", "First snapshot file (Snapshot A)")
	cmd.Flags().StringVar(&snapshotDiffBFlag, "snapshot-b", "", "Second snapshot file (Snapshot B)")
	cmd.Flags().IntVar(&snapshotDiffOffsetFlag, "offset", -1, "Start offset to compare (default: compare all)")
	cmd.Flags().IntVar(&snapshotDiffLengthFlag, "length", 0, "Number of bytes to compare (default: all from offset)")
	cmd.Flags().IntVar(&snapshotDiffContextFlag, "context", 16, "Bytes of context to show around changes")
	return cmd
}
