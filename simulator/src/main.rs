...
use soroban_env_host::{ xdr::{ Operation, OperationBody }, Host, HostError };
...
fn generate_lcov_report(coverage: &CoverageTracker, source_file: &str) -> String {
    let mut functions: Vec<(&str, u64)> = coverage.invoked_functions
        .iter()
    ...
    for op in operations {
        ...
        match &op.body {
            OperationBody::InvokeHostFunction(invoke_op) => {
                ...
                if
                    let Some(mock_result) = check_signature_verification_mocks(
                        &request,
...
fn scval_to_xdr_base64(val: &soroban_env_host::xdr::ScVal) -> String {
    base64::engine::general_purpose::STANDARD.encode(
        val.to_xdr(soroban_env_host::xdr::Limits::none()).unwrap_or_default()
    )
}
...
fn mocked_required_fee_stroops(
    ...
    if let Some(base_fee) = request.mock_base_fee {
        enabled = true;
        required_fee = required_fee.saturating_add(
            (base_fee as u64).saturating_mul(operations_count as u64)
        );
    }
...
fn main() {
    ...
    let envelope = match base64::engine::general_purpose::STANDARD.decode(&request.envelope_xdr) {
        Ok(bytes) =>
            match
                soroban_env_host::xdr::TransactionEnvelope::from_xdr(
                    bytes,
        ...
        match base64::engine::general_purpose::STANDARD.decode(&request.result_meta_xdr) {
            Ok(bytes) => {
                if bytes.is_empty() {
                    ...
                    match
                        soroban_env_host::xdr::TransactionResultMeta::from_xdr(
                            &bytes,
                            soroban_env_host::xdr::Limits::none()
                        )
                    {
            ...
            Err(e) => {
                eprintln!(
                    "Warning: Failed to decode ResultMeta Base64: {e}. Proceeding with empty storage."
                );
            }
        }
    }
    ...
    let resolved_entries: Option<std::collections::HashMap<String, String>> = if
        let Some(b64) = &request.ledger_entries_zstd
    {
    ...
    if request.profile.unwrap_or(false) {
        ...
        if
            let Err(e) = inferno::flamegraph::from_reader(
                &mut options,
    ...
    if request.enable_coverage {
        let source_file = request.wasm_path.clone().unwrap_or_else(|| "contract.wasm".to_string());
    ...
    match result {
        Ok(Ok(exec_logs)) => {
            let (events, _diag_evs): (Vec<String>, Vec<DiagnosticEvent>) = match host.get_events() {
                Ok(evs) => {
                    let raw_events: Vec<String> = evs.0
                        .iter()
                        .map(|e| format!("{:?}", e))
                        .collect();
                    let diag_events: Vec<DiagnosticEvent> = evs.0
                        ...
                        .map(|event| {
                            ...
                            let contract_id = event.event.contract_id
                                .as_ref()
                                .map(contract_id_to_hex);

                            let (topics, data) = match &event.event.body {
                                soroban_env_host::xdr::ContractEventBody::V0(v0) => {
                                    let topics: Vec<String> = v0.topics
                                        .iter()
                                        .map(scval_to_xdr_base64)
                                        .collect();
                ...
                Err(_) =>
                    (vec!["Failed to retrieve events".to_string()], Vec::<DiagnosticEvent>::new()),
            };
            ...
            if
                let Some(required_fee) = mocked_required_fee_stroops(
                    &request,
                ...
                )
            {
                ...
                final_logs.push(
                    format!("Mock fee check: declared={} required={}", declared_fee, required_fee)
                );
                ...
                if declared_fee < required_fee {
                    ...
                    if let Ok(json) = serde_json::to_string(&response) {
                        ...
                        println!(
                            "{{\"status\": \"error\", \"error\": \"Internal serialization error\"}}"
                        );
                    }
                    return;
                }
        ...
        Ok(Err(host_error)) => {
            ...
            let _source_location = wasm_trace
                .offset()
                .and_then(|offset| {
                    source_mapper.as_ref().and_then(|m| m.map_wasm_offset_to_source(offset))
                });
            ...
            for event in &diagnostic_events {
                ...
                if
                    combined_text.contains("panicked") ||
                    ...
                    combined_text.contains("Trap")
                {
                    ...
                    if
                        combined_text.contains("core/src/panicking.rs") ||
            ...
            let details = if let Some(ref point) = user_panic_point {
                ...
                format!("Contract execution failed with host error: {:?}", host_error)
            };
            ...
            let source_location = if
                let (Some(offset), Some(mapper)) = (wasm_offset, &source_mapper)
            {
...
pub fn decode_error(raw: &str) -> String {
        ...
        lower.contains("storage get failed")
    {
        ...
        if let Some(key) = key_hint {
            return format!("Missing ledger entry — Key ID: {} — the contract referenced a key that does not exist in the current ledger state.", key);
        }
...
mod tests {
    ...
    fn test_decode_vm_traps() {
        assert!(
            decode_error("Error: Wasm Trap: out of bounds memory access").contains(
                "VM Trap: Out of Bounds Access"
            )
        );
    ...
    fn test_missing_ledger_entry_includes_key_id() {
        let raw =
            "HostError: storage get failed LedgerKey(ContractData(hash=abc123, key=Symbol(\"balance\")))";
    ...
    fn test_missing_ledger_entry_no_key_falls_back() {
        ...
        assert_eq!(
            msg,
            "Missing ledger entry — the contract referenced a key that does not exist in the current ledger state."
        );
    }
    ...
    fn test_generate_lcov_report_contains_function_hits() {
        let mut coverage = CoverageTracker::default();
        coverage.invoked_functions.insert("InvokeContract::\"transfer\"".to_string(), 3);
        coverage.invoked_functions.insert("InvokeContract::\"init\"".to_string(), 1);
